<section class="max-w-8xl mx-auto mt-20 text-center">

    <div class="space-x-8 lg:space-y-0 lg:space-x-4 mt-4 mb-12">

        <h1 class="font-bold text-4xl">Hi, There!</h1><br>

        <h1 class="text-2xl">Am Sammy Mutisya Web/Application Developer from Nairobi, Kenya.</h1>

        <h1 class="text-2xl">I love programming, as well as playing soccer, travelling and learning new things!</h1>

        <h1 class="text-2xl">Feel free to get in touch or take a look at my portfolio below.</h1>
       
    </div>
</section><?php /**PATH C:\Laravel_Projects\Mutisya_portfolio\resources\views/posts/biosection.blade.php ENDPATH**/ ?>